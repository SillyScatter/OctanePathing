...
public class Util {
    ...
    public static final String LOG_PREFIX = "&8[&eOctanePathing&8]"; //Only affects messages sent to console
    ...
    public static void spawnParticle(Location location, Particle particleType) {
        location.getWorld().spawnParticle(particleType, location, 1, 0, 0, 0, 0);
    }

    public static ShulkerBullet spawnStationaryShulkerBullet(Location location) {
        ShulkerBullet shulkerBullet = (ShulkerBullet) location.getWorld().spawnEntity(location, EntityType.SHULKER_BULLET);
        shulkerBullet.setVelocity(new Vector());
